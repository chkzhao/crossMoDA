...
class StyleGAN2Generator(nn.Module):
    def __init__(self, input_nc, output_nc, ngf=64, use_dropout=False, n_blocks=6, padding_type='reflect', no_antialias=False, opt=None, minimum=False,):
        super().__init__()
        self.opt = opt
        self.minimum = minimum
        self.encoder = StyleGAN2Encoder(input_nc, output_nc, ngf, use_dropout, n_blocks, padding_type, no_antialias, opt)
        self.bn = ccbn(int(round(128 * ngf / 32)))
        self.decoder = StyleGAN2Decoder(input_nc, output_nc, ngf, use_dropout, n_blocks, padding_type, no_antialias, opt)

    def forward(self, input, layers=[], encode_only=False, d=None, alpha=None):
        feat, feats = self.encoder(input, layers, True)
        if self.minimum:
            feat = self.bn(feat, d, alpha)
