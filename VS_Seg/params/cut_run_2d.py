...
class CUTModel_2d(ABC):
    ...
    def data_dependent_initialize(self, data):
        ...
        if self.opt.transfer_seg and self.opt.load_dict:
            path = os.path.join("../results", self.opt.dataset, self.opt.model, str(
                self.opt.intensity) + 'Transfer_' + 'DA_INS_' + self.opt.results_folder_name)
            model_path = os.path.join(path, "model")
            self.netF.load_state_dict(torch.load(os.path.join(model_path, "250_epoch_F.pth")))
    ...
    def forward(self):
        ...
        with self.autocast():
            ...
            index = np.ndarray.tolist(np.random.choice(self.fake_B.shape[4], 10, replace=False))
            self.fake_B = (self.fake_B[..., index]).permute(4,0,1,2,3).flatten(0,1)
            self.real_A = (self.real_A[..., index]).permute(4, 0, 1, 2, 3).flatten(0, 1)
            self.real_B = (self.real_B[..., index]).permute(4, 0, 1, 2, 3).flatten(0, 1)
