...
class ResnetGenerator(nn.Module):
    ...
    def __init__(self, input_nc, output_nc, ngf=64, norm_layer=nn.BatchNorm2d, use_dropout=False, n_blocks=6, padding_type='reflect', no_antialias=False, no_antialias_up=False, opt=None, minimum=False):
        ...
        l_down = len(model_down)
        l_up = len(model_up)

        for i in range(l_down):
            model_down[i] = SN(model_down[i])

        for i in range(l_up):
            model_up[i] = SN(model_up[i])
    ...
    def forward(self, input, layers=[], encode_only=False, d=None):
        if -1 in layers:
