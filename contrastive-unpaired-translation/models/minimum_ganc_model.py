...
class MinimumGANCModel(BaseModel):
    """
    ...
    def __init__(self, opt):
        ...
        self.loss_names = ['D_A', 'G_A', 'NCE_A', 'D_B', 'G_B', 'NCE_B', 'minimum_reg']
        ...
        self.netG = networks.define_G(opt.input_nc, opt.output_nc, opt.ngf, opt.netG, opt.normG, not opt.no_dropout,
                                      opt.init_type, opt.init_gain, opt.no_antialias, opt.no_antialias_up, self.gpu_ids, opt, minimum=True)
    ...
    def backward_G(self):
        ...
        if self.opt.nce_idt and self.opt.lambda_NCE > 0.0:
            self.loss_NCE_B = self.calculate_NCE_loss(self.real_B, self.rec_B)
        else:
            self.loss_NCE_B = 0.0

        if self.opt.nce_idt and self.opt.lambda_NCE > 0.0:
            self.loss_NCE_A = self.calculate_NCE_loss(self.real_A, self.rec_A)
        else:
            self.loss_NCE_A = 0.0
        ...
        # combined loss and calculate gradients
        self.loss_G = self.loss_G_A + self.loss_G_B + (self.loss_NCE_A + self.loss_NCE_B)*0.5 + self.loss_minimum_reg * 0.001  # + self.loss_cycle_A + self.loss_cycle_B
